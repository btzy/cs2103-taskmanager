...
[completedemo1]
.Result after one `clear` command and two `add` commands
[#img-completedemo3]
[caption="Figure 3.3.1: "]
image::userguide/completedemo_1.png[CompleteDemo1, width="400"]

[completedemo2]
.Result of the `complete 2` command
[#img-completedemo2]
[caption="Figure 3.3.2: "]
image::userguide/completedemo_2.png[CompleteDemo2, width="400"]

[completedemo3]
.Result of the `complete 1` command
[#img-completedemo3]
[caption="Figure 3.3.3: "]
image::userguide/completedemo_3.png[CompleteDemo3, width="400"]
...
.Result of `filter assignment` command (notice that the keyword "assignment" matches task name, tags, and attachments)
[#img-filterdemo_simplified]
[caption="Figure 3.6.2.1: "]
image::userguide/filterdemo_simplified.png[FilterDemo_Simplified, width="300"]
...
* `filter homework`
Returns a subset of the current list of tasks that have a name, tag, or attachment that matches "homework".  Note that "homework" cannot be interpreted as a valid date, so the deadline field is ignored.

* `filter assignment CS2103`
Returns a subset of the current list of tasks that have a name, tag, or attachment that matches "assignment" and a name, tag, or attachment that matches "CS2103".  Note that neither "assignment" or "CS2103" can be interpreted as a valid date, so the deadline field is ignored for both keywords.

* `filter badminton utown 3/10/2018`
Returns a subset of the current list of tasks that have a name, tag, or attachment that matches "badminton"; a name, tag, or attachment that matches "utown"; and a name, tag, attachment, or deadline that matches "3/10/2018".  Note that any deadline on or before 3^rd^ October 2018 will match "3/10/2018".

* `filter 1/10/2018`
Returns a subset of the current list of tasks that have a name, tag, attachment, or deadline that matches "3/10/2018".  Note that any deadline on or before 1^st^ October 2018 will match "1/10/2018".

// tag::filter-field[]
==== Controlling the field being matched

image::FilterUnitDiagramUG.png[width="200",role="thumb right"]

Each keyword can instead be written as a _full predicate_, which is a string of the following format: `<key><operator><phrase>`. The diagram on the right is an example of a typical full predicate.
...
[NOTE]
In the examples below, when we say that a task _has_ a test phrase, we mean that the task has a name, tag, or attachment that contains the test phrase as a substring.

Examples:

* `filter assignment|CS2101`
Returns a subset of the current list of tasks that has "assignment" or "CS2101".

* `filter ! t:CS2101`
Returns a subset of the current list of tasks that do not have the tag "CS2101".

* `filter cs & ((present & f=7) | meeting)`
Returns a subset of the current list of tasks that has "cs" and either (1) has "present" and repeats exactly once a week, or (2) has "meeting". +
The following screenshot is an example of a list of tasks that could be displayed by the above example:

[filterdemo_complex]
.Result of `filter cs & ((present & f=7) | meeting)` command
[#img-filterdemo_complex]
[caption="Figure 3.6.4.1: "]
image::userguide/filterdemo_complex.png[FilterDemo_Complex, width="300"]
...
image::FilterUnitDiagramSetUG.png[width="250",role="thumb right"]
...
[filterdemo_error1]
.Possible error result #1
[#img-filterdemo_error1]
[caption="Figure 3.6.6.1: "]
image::userguide/filterdemo_error1.png[FilterDemo_Error1, width="400"]

[filterdemo_error2]
.Possible error result #2
[#img-filterdemo_error2]
[caption="Figure 3.6.6.2: "]
image::userguide/filterdemo_error2.png[FilterDemo_Error2, width="400"]
